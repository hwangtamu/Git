...
def write(f):
    ...
    writer.writerow(['f1','f2','f3','f4','f5','f6','f7','f8','f9','f10','f11','f12','f13','f14','f15','f16','f17','f18','f19','f20','f21','f22'])
...
def feature(data):
    ...
    for i in range(len(x)-1):
        tmp_raw += math.atan2((x[i+1]*y[i]-x[i]*y[i+1]),(x[i]*x[i+1]+y[i]*y[i+1]))
    ...
    tmp = sorted(speed)[-1]
    features.append(abs(math.pi/4-features[3]))    #12
    tmp = 0
    for i in range(len(x)-1):
        if abs(math.atan2((x[i+1]*y[i]-x[i]*y[i+1]),(x[i]*x[i+1]+y[i]*y[i+1]))) < 19*math.pi/180:
            tmp += math.atan2((x[i+1]*y[i]-x[i]*y[i+1]),(x[i]*x[i+1]+y[i]*y[i+1]))
    features.append(tmp)    #13
    features.append(features[8]/features[7])    #14
    features.append(features[7]/features[4])    #15
    features.append(features[7]/features[2])    #16
    features.append(features[4]/features[2])    #17
    features.append((y_max-y_min)*(x_max-x_min)) #18
    features.append(math.log(features[17])) #19
    features.append(features[8]/features[9])    #20
    features.append(math.log(features[7]))   #21
    features.append(math.log1p(features[11])) #22
